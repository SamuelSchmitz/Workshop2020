...
if __name__ == '__main__':
	...
	parser.add_option("--nearby_atom_cutoff",dest="nearby_atom_cutoff",help="SC distance cutoff to define a residue as part of the interface. If any SC atom from a residue on one side is within this cutoff of a residue on the other side it's considered to be in the interface. Default=5.0",default=5.0)
	...
	side_dict = {}
	side_dict[ '1' ] = sorted( side1, key=lambda res: ( int(res[0]), res[1] ) )
	side_dict[ '2' ] = sorted( side2, key=lambda res: ( int(res[0]), res[1] ) )
